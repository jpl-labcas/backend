...
if __name__ == '__main__':
    ...
    try:
        ...
        dataset_id = metadata['DatasetName'].replace(' ','_') 
        
    # generate Collection id from Collection name
    collection_id = metadata['CollectionName'].replace(' ','_')
    ...
    print "Starting to traverse rootDir=%s" % rootDir
    ...
    labcasClient.traverseDir(rootDir, metadata, inPlace=inPlace, debug=debug)

    # query the product types from the XML/RPC File Manager interface
    if debug:
       labcasClient.getProductTypeByName(collection_id)
    
    # or equivalently
    if debug:
       labcasClient.getProductTypeById("urn:edrn:%s" % collection_id)
    
    # list all products for given product type
    if debug:
       labcasClient.listProducts(collection_id)
