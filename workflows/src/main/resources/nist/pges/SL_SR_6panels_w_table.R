#!/usr/bin/env Rscript
#setwd("/Users/aam/work/Projects/EDRN/NISTmiRNA/Curated_Site_Data/Take3")
...
#####################################
SL_SR6 <- function (SitePlat, Round) {
...
source("TargetPlot_D.R")

TargetPlot<-makeTargetPlot(SitePlat, indf = Lab_df_tall_recast, correction=c(Bf,Lf,Pf))

# BarChart of TSI Classes
DetectedBarGraph <- ggplot(subset(Mix1_v_Mix2, !is.na(TSI_Class)), aes(TSI_Class, fill=TSI_Class)) +
...
# write.csv(cbind(MetricsGrid1, MetricsGrid2), file = paste(SitePlat,"_",Round,"_6panel.csv", sep=""))

# Final Multi-Panel Figure for screen
# grid.arrange(MetricsTable, MetricsTable_R,
#              PredictedPlot, TargetPlot,
...
ggsave(file=paste(SitePlat,"_",Round,"_6panel.pdf", sep=""), g, width = 7.5, height = 10)

}
#############
SL_SR6(SitePlat, Round)
