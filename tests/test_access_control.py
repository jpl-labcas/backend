...
from jpl.labcas.backend.services.access_control import (
    NO_ACCESS_FILTER,
    build_owner_principal_filter,
    user_may_download,
)
...
def test_build_owner_principal_filter_authenticated_without_groups() -> None:
    """Authenticated users with no groups may still see public metadata."""
    settings = Settings(
        solr_url="http://localhost:8983/solr",
        public_owner_principal="public",
        super_owner_principal=None,
...
def test_build_owner_principal_filter_no_principals_returns_no_access() -> None:
    settings = Settings(
        solr_url="http://localhost:8983/solr",
...
def test_user_may_download_requires_groups() -> None:
    assert user_may_download(SecurityContext(subject="uid=tester,ou=users,dc=example,dc=com", groups=["g1"]))
    assert not user_may_download(SecurityContext(subject="uid=tester,ou=users,dc=example,dc=com", groups=[]))
    assert not user_may_download(SecurityContext(subject=GUEST_USER_DN, groups=[]))
